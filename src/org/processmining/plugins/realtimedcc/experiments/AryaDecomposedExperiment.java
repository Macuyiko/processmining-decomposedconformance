...
public class AryaDecomposedExperiment {
	...
	public static String csvPath = basePath+"aryaDecomposed-noise.csv";
	public static boolean groupLog = false;
	...
	private static void runExperiment(AcceptingPetriNetArray a, XLog log) {
		...
		// Give experiment chance to setup
		timer.start();
		...
		for (int index = 0; index < size; index++) {
			XLog grouped = null;
			if (groupLog)
				grouped = new GroupedXLog(eventLogArray.getLog(index)).getGroupedLog();
			else
				grouped = eventLogArray.getLog(index);
			for (int t = 0; t < grouped.size(); t++) {
				XTrace trace = grouped.get(t);
				if (trace.size() == 0) continue;
				ProcessReplayModel<Transition, XEventClass, Marking> replayModel = null;
				
				replayModel = new AryaPetrinetReplayModel(
