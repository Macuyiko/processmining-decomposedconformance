...
public class ReplayEventLogArrayOnAcceptingPetriNetArrayDialog extends JPanel {
	...
	public ReplayEventLogArrayOnAcceptingPetriNetArrayDialog(EventLogArray logs, AcceptingPetriNetArray nets,
	...
	public JPanel getPanel(int n) {
		...
		if (n == 0) {
			...
			mapping = new TransEvClassMapping(selectedClassifier.get(0), invisibleActivity);
			for (int i = 0; i < nets.getSize(); i++) {
				parameters.setMapping(i, new TransEvClassMapping(selectedClassifier.get(0), invisibleActivity));
				...
				parameters.setReplayParameters(i, parameters.createReplayParameters(activities.values(), invisibleActivity,
						nets.getNet(i)));
			}
