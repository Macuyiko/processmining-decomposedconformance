...
public class TransEvClassMappingPanel extends JPanel {
	...
	public TransEvClassMappingPanel(Set<XEventClass> activities, final TransEvClassMapping map) {

		XEventClass invisibleActivity = new XEventClass(INVISIBLE, map.size());
				...
				public void actionPerformed(ActionEvent e) {
					for (int j = 0; j < comboBoxes.length; j++) {
						...
						if (comboBox.equals(source)) {
							...
							for (Transition transition : map.keySet()) {
								if (transition.getLabel().equals(transitionList.get(j).getLabel())) {
